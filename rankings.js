...
// Added your team mapping for the logo fallback
...
window.switchTab = async function(type, col, el) {
    // If switching category, reset to QB (col 0)
    if (type !== state.type) {
        state.type = type; state.col = 0;
        // Update UI: force QB button active
        ...
        document.querySelector(`[onclick*="switchTab('${type}', 0"]`).classList.add('active');
    } else {
        state.col = col;
        el.parentElement.querySelectorAll('.bubble').forEach(b => b.classList.remove('active'));
...
function renderList() {
    ...
    masterRankings.forEach(row => {
        ...
        if (name && !name.toUpperCase().includes("PLAYER")) {
            count++;
            const playerImg = photoBank[name] || normalizedBank[normalize(name)] || teamLookup[team];
            html += `<a href="https://www.fantasypros.com/nfl/players/${name.toLowerCase().replace(/ /g, '-')}.php" target="_blank" class="fnp-row">
                ...
                <div style="flex-grow:1"><span class="fnp-name">${name}</span><span class="fnp-meta">${team || ''}</span></div>
            </a>`;
        }
    });
